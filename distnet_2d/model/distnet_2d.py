...
from ..utils.losses import weighted_binary_crossentropy, weighted_loss_by_category, balanced_category_loss, edm_contour_loss, balanced_background_binary_crossentropy, MeanSquaredErrorChannel, l2, get_grad_weight_fun, PseudoHuber
...
class DistnetModel(Model):
    def __init__(self, *args, spatial_dims,
        ...
        edm_loss= PseudoHuber(),
        center_loss = PseudoHuber(),
        displacement_loss = PseudoHuber(),
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=self.use_ags or self.print_gradients) as tape:
            ...
            losses["loss"] = loss
            # if self.use_rgs: # if RGS: loss weights are not taken into acount
            #     loss = 0.
            #     for k, l in losses.items():
            #         loss += l
            #     if mixed_precision:
...
def get_distnet_2d_erf3(input_shape, # Y, X
            encoder_settings:list,
            feature_settings: list,
            feature_blending_settings: list,
            feature_decoder_settings:list,
