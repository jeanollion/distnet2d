...
from ..utils.losses import weighted_binary_crossentropy, weighted_loss_by_category, balanced_category_loss, balanced_background_binary_crossentropy, MeanSquaredErrorChannel, l2, get_grad_weight_fun, PseudoHuber
...
class DistnetModel(Model):
    def __init__(self, *args, spatial_dims,
        edm_loss_weight:float=1, edm_lovasz_loss_weight:float=1,
        center_loss_weight:float=1, center_unicity_loss_weight:float=0,
        displacement_loss_weight:float=1,
        center_displacement_loss_weight:float=0, center_displacement_grad_weight_center:float=1e-1, center_displacement_grad_weight_displacement:float=1e-1, # ratio : init: center/motion = 10-100 . trained : motion/center = 10-100
...
def get_distnet_2d_erf2(input_shape, # Y, X
        ...
        combine_features_op = Combine(filters=combine_filters, compensate_gradient = True, name="CombineFeatures")
        if attention:
            attention_op = SpatialAttention2D(positional_encoding="2D", name="Attention") # TODO attention op + skip per frame pair + add convolutions
            attention_combine = Combine(filters=combine_filters, compensate_gradient = True, name="AttentionCombine")
            attention_skip_op = Combine(filters=combine_filters, name="AttentionSkip")
        for f in feature_blending_settings:
            if "filters" not in f or f["filters"]<0:
        ...
        if attention:
            ...
            if long_term:
                ...
                if next:
                    for c in range(frame_window+2, n_chan):
                        attention_result.append(attention_op([all_features[frame_window], all_features[c]]))
            attention_result = attention_combine(attention_result)
            combined_features = attention_skip_op([attention_result, combined_features])
...
def get_distnet_2d_erf4(input_shape, # Y, X
            encoder_settings:list,
            feature_settings: list,
            feature_blending_settings: list,
            feature_decoder_settings:list,
