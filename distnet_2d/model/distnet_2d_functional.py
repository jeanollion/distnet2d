...
def get_distnet_2d(input_shape,
        ...
        decoder_layers = [decoder_op(**parameters, size_factor=contraction_per_layer[l_idx], conv_kernel_size=3, mode=upsampling_mode, skip_combine_mode=skip_combine_mode, skip_mode=first_skip_mode if l_idx==0 else None, activation="relu", layer_idx=l_idx) for l_idx, parameters in enumerate(decoder_settings)]
