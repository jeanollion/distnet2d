class ASABlend():
    def __init__(self, filters:int = 192, up_filters:int=96): # up filters was 94
...
class ASABlendD2():
    def __init__(self, filters:int = 128, up_filters:int=64):
...
class ASABlendD2D():
    def __init__(self, filters:int = 128):
        self.name = f"asa-blend-d2_dec-{filters}"
        self.attention = True
        self.combine_kernel_size = 1
        self.downsampling_mode="maxpool_and_stride"
        self.upsampling_mode ="tconv"
        self.encoder_settings = [
            ...
            [
                {"filters":32, "op":"conv", "weighted_sum":True, "weight_scaled":False, "dropout_rate":0}, # was resconv2D
                {"filters":32, "op":"conv", "weighted_sum":True, "weight_scaled":False, "dropout_rate":0}, # was resconv2D
                {"filters":filters, "downscale":2, "weight_scaled":False, "dropout_rate":0}
            ]
        ]
