...
if not seg:
    if True:
        ...
        dn = get_distnet_2d(
            arch=architectures.TemPyD2(frame_window=10, spatial_dimensions=(64, 32), filters=128, self_attention=16, attention_filters=64,
        ...
        dn.compile()
        frame_index = tf.reshape(tf.range(21), [1, 1, 1, 21])
        out = dn([tf.zeros(shape=(1, 64, 32, 21)), frame_index]) # frame aware case
    ...
    if False:
        ...
        print(f"shape : x={[xx.shape for xx in x]} y={[yy.shape for yy in y]}")
        fw = 5
        n_frames = fw * 2 + 1
        n_frame_pairs = fw * 2
        ...
        ax[1].imshow(x[1][0,...,fw], interpolation="nearest", aspect='equal', origin="upper")
        ax[2].imshow(x[2][0,...,fw], interpolation="nearest", aspect='equal', origin="upper")
        ax[3].imshow(x[3][0,...,fw], interpolation="nearest", aspect='equal', origin="upper")
